...
## Unreleased
- **A Grok repair that is still producing an answer no longer dies on the 10-minute stall.** The repair idle now restarts only for a parsed progress event or an incomplete tail that already names one. SSE comments and `response.in_progress`, including a sliced lifecycle frame, do not keep it alive. While a tool call, final answer, or unfinished progress frame is withheld, the forwarder emits U+2060 on `reasoning_content` at most once per `CODEX_ROUTER_GROK_REPAIR_KEEPALIVE_MS` (default 30s; `0` emits on every such event). The first Grok Responses transform rewrites that delta to `response.in_progress` and strips the character from reasoning snapshots. The primary-attempt stall and the transport bounds are unchanged.
